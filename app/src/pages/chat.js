...
import { Layout, Button, Navigation, Modal, Typography } from '../components';

export default function Chat() {
    // Get auth context datas / methods
    const auth = useAuth();
    const [toggleUsersModal, setToggleUsersModal] = useState(false);
    const [toggleAlertsModal, setToggleAlertModal] = useState(false);

    useEffect(() => {
        const timer = new IdleTimer({
            timeout: 600, //expire after 10 mins of inactivity
        ...
        // If user use the app, clear the timer
        return () => {
            timer.cleanUp();
        };
    ...
    // Redirect to Signin if user hasn't signed in
    return auth.localStorageUser ? (
        <>
            ...
            <Modal
                title="Uh oh."
                toggleModal={toggleAlertsModal}
                setToggleModal={setToggleAlertModal}
                alert={true}
            >
                <Typography.BodyLarge>👋 Hey, <b>{auth.localStorageUser.username}</b></Typography.BodyLarge>
                <Typography.Body>Looks like you've been away for a while, consequently, <strong>you've been disconnected.</strong></Typography.Body>
                <Typography.Body>Click the button bellow to reconnect.</Typography.Body>
                <hr />
